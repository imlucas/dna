...
my $IS_BIG_ENDIAN = unpack("h*", pack("s", 1)) =~ /01/; #Â See perlport
...
sub get_directory {
    ...
    if ($self->search_tag($tag_name, $tag_number)) { # Found!
        ...
        if ($DirEntry{DATA_SIZE} > 4) {
            ...
            #Â (the current file handle position)
            read($self->{'_FH'}, $raw_data, $DirEntry{DATA_SIZE});
        }
        $DirEntry{DATA_ITEM} = $raw_data; #Â Return raw data
...
sub write_tag {
    ...
    if ($self->search_tag($tag_name, $tag_number)) {
        ...
        # Write data #
        ##############
        seek($self->{'_FH'}, -8, 1); #Â Go back to numelements field...
        ...
        # in the current position
        elsif ($n_bytes <= $data_size) { #Â It fits!
            ...
            print { $self->{'_FH'} } $packed_data; #Â Append new data
...
=head2 buffer_tray_temperature()
  ...
  Returns   : The buffer tray heater temperature in Â°C;
              () if the data item is not in the file.
  ABIF Tag  : BufT1
  ABIF Type : short array
  File Type : ab1
...
=head2 detector_heater_temperature()
  ...
  Returns   : The detector cell heater temperature in Â°C;
              undef if the data item is not in the file.
  ABIF Tag  : DCHT1
  ABIF Type : short
  File Type : ab1
...
=head2 run_temperature()
  ...
  Returns   : The run temperature setting in Â°C;
              undef if the data item is not in the file.
  ABIF Tag  : Tmpr1
  ABIF Type : long
  File Type : ab1, fsa
...
=head2 temperature()
  ...
  Returns   : The temperature, measured in Â°C
              () if the data item is not in the file.
  ABIF Tag  : DATA8
  ABIF Type : short array
  File Type : ab1, fsa
...
sub clear_range_start {
    ...
    my $N = scalar(@$qv_ref);
    return -1 if ($N < $window); #Â Not enough quality values
...
sub clear_range_stop {
    ...
    my $j; #Â Points to the leftmost element of next window
    my $n = 0; # Number of bad quality bases
    ...
    return -1 if ($n >= $bad_bases); #Â No clear range
    return $j + $window;
}
...
sub contiguous_read_length {
    ...
    my $crl = 0; #Â 1 if we are inside a crl, 0 otherwise
    my $start = 0;
    my $new_start = 0;
    my $stop = 0;
...
sub num_low_quality_bases {
    ...
    else { #Â Count all the quality values
